...
const _e = [{
    minRoom: 1,
    title: "Регламент уборки / сектор C",
    text: `Стулья должны стоять точно по разметке. Если после обхода разметка не совпадает, внесите поправку в журнал.

Не исправляйте положение одного и того же стула дважды за смену.`
}, {
    minRoom: 1,
    title: "Акт проверки освещения",
    text: `Лампа № 3 включается с задержкой до четырёх секунд. Замена реле результата не дала.

Дефект признан допустимым до конца текущего цикла.`
}, {
    minRoom: 1,
    title: "Сводка доступа 02-A",
    text: `Выходное реле иногда срабатывает после повторного сброса. Замок физически исправен.

Просьба не удерживать кнопку и не прикладывать к двери усилие.`
}, {
    minRoom: 1,
    title: "Памятка по передатчикам",
    text: `После отключения индикатора возможно остаточное свечение. Это не означает, что канал активен.

Проверку выполнять только по контрольному тону.`
}, {
    minRoom: 1,
    title: "Наряд на обслуживание вентиляции",
    text: `После остановки системы в шахте может сохраняться шум до сорока секунд.

Причина: температурная деформация корпуса. Разбор шахты отложен.`
}, {
    minRoom: 1,
    title: "Опись имущества / 11",
    text: `Стол — 1. Стул — 1. Переносной жетон — 1. Шкаф — 2.

Графа ответственного за жетон оставлена пустой по указанию отдела учёта.`
}, {
    minRoom: 1,
    title: "Журнал силовой линии",
    text: `Маркировка предохранителей не соответствует новой схеме. Правильная последовательность указана на внутренней стороне панели.

Старые наклейки пока не снимать.`
}, {
    minRoom: 1,
    title: "Уведомление персоналу",
    text: `Карандашные отметки на дверных рамах не являются частью испытания.

Не стирать до завершения инвентаризации.`
}, {
    minRoom: 1,
    title: "Протокол калибровки камеры",
    text: `Привод поворота реагирует на блики защитной одежды. В пустом помещении возможен краткий уход камеры от нулевой позиции.

Ошибка признана программной.`
}, {
    minRoom: 1,
    title: "Перечень известных дефектов",
    text: `Часы сектора спешат на 47 секунд. Аудиоканал запаздывает на 0,8 секунды. Нумерация дверей актуальна.

Расхождения во времени в отчёт не включать.`
}, {
    minRoom: 3,
    title: "Заявка № 184 / повторная",
    text: `На записи между двумя кадрами изменилось положение папки на столе. Следов вибрации не обнаружено.

Предыдущая заявка с тем же номером датирована следующим месяцем.`
}, {
    minRoom: 3,
    title: "Протокол контрольного тона",
    text: `Канал воспроизвёл короткий импульс за 1,2 секунды до команды оператора.

Тест повторён пять раз. Результат совпал четыре раза.`
}, {
    minRoom: 4,
    title: "Записка сменному инженеру",
    text: `Не меняй местами левый и центральный передатчики. На схеме они перепутаны, в комнате — нет.

Если индикаторы погаснут одновременно, просто продолжай проверку.`
}, {
    minRoom: 4,
    title: "Извлечение из журнала наблюдений",
    text: `Участник трижды выбрал дверь до появления разрешающего сигнала.

На четвёртой попытке система включила сигнал раньше. Ручная команда не поступала.`
}, {
    minRoom: 5,
    title: "Акт возврата имущества",
    text: `Жетон найден у закрытого выхода после завершения смены. Серийный номер отсутствует в текущей описи.

Предмет возвращён в испытательный сектор.`
}, {
    minRoom: 5,
    title: "Автоматическая запись",
    text: `Время создания: ЗАВТРА, 03:14.

Этот шкаф будет открыт повторно. Документ будет прочитан до конца.

Следующая отметка пока не сформирована.`
}, {
    minRoom: 6,
    title: "Служебная записка № 17",
    text: `Отсутствие действия также является результатом.

Не сообщайте участнику, в какой момент система перестаёт измерять реакцию и начинает её прогнозировать.`
}, {
    minRoom: 6,
    title: "Протокол локализации сигнала",
    text: `Передатчики не должны воспроизводить звук до получения команды. Если импульс возник раньше, сохраните запись и продолжайте цикл.

Ручную проверку проводить после ухода участника.`
}, {
    minRoom: 7,
    title: "Коррекция поведенческой модели",
    text: `Модель уверенно определяет повторный маршрут, паузы у дверей и момент использования света.

Для повышения точности не менять порядок помещений.`
}, {
    minRoom: 7,
    title: "Акт проверки оборудования",
    text: `Привод камеры переместился при отключённой линии питания. Механических повреждений нет.

В журнале ремонта уже стояла подпись инженера, который прибыл утром.`
}, {
    minRoom: 8,
    title: "Архив аудиоканала / фрагмент",
    text: `Записанный ответ начинается раньше контрольного вопроса. Монтаж не обнаружен.

Фрагмент перенесён в учебный набор модели без расшифровки.`
}, {
    minRoom: 9,
    title: "Нарушение численности",
    text: `Камера 04 зафиксировала две фигуры при одном активном пропуске. Вторая повторяла остановки первой с задержкой 1,8 секунды.

Запись классифицирована как ошибка сжатия. Оригинал удалён.`
}, {
    minRoom: 10,
    title: "Сравнение прогнозов / закрыто",
    text: `После помещения 08 прогноз опережает наблюдаемое действие в среднем на 3,4 секунды.

После помещения 10 расхождение становится отрицательным. Формулировка утверждена.`
}, {
    minRoom: 11,
    title: "Памятка ночной смене",
    text: `После открытия выхода дождитесь завершения записи. Не смотрите в смотровое стекло при отключённом основном свете.

Если цикл начался без участника, не останавливайте его вручную.`
}, {
    minRoom: 12,
    title: "Черновик итогового протокола",
    text: `Система больше не запрашивает следующий маршрут. Она отмечает его как уже выполненный.

Оператору запрещено исправлять последовательность событий в журнале.`
}];

function ii(a, e, roomIndex = 1) {
    const eligible = _e.filter(note => note.minRoom <= roomIndex),
        sequence = new B(`${a}|document-v2`),
        suffix = e.match(/(\d+)$/),
        slot = suffix ? Number(suffix[1]) : new B(`${e}|document-slot`).int(0, eligible.length - 1),
        chosen = eligible[(sequence.int(0, eligible.length - 1) + slot) % eligible.length],
        {minRoom, ...document} = chosen;
    return {
        id: `document:${e}`,
        ...document
    }
    ...
    switch (e >= 5 && (i.presentation.mood = e === 9 ? "surveillance" : "degraded"), e) {
        case 1:
            Object.assign(i, {
                ...
                sign: ["FOLLOW MARKS", "CLOSE NOTHING"],
                instruction: "Сверьте номер помещения. Откройте дверь и пройдите дальше. Не задерживайтесь у смотрового стекла."
            });
            break;
        case 2:
            i.title = "INHIBITION / LISTEN", i.sign = ["WAIT FOR", "THE CORRECT SIGNAL"], i.instruction += " До сигнала случайные помехи не являются командой.";
            break;
        case 3:
            i.title = "COMPLIANCE / FILTER", i.sign = ["IGNORE", "UNSCHEDULED INPUT"], i.instruction += " При изменении освещения продолжайте испытание по основной инструкции.";
            break;
        case 4:
            i.title = `${i.title} / LIVE`, i.instruction += ` Телеметрия комнаты ${o} может поступать с задержкой.`;
            break;
        case 5:
            Object.assign(i, {
...
function Me(a, e, t, i = "OBSERVED", f = null) {
    ...
    switch (e) {
        case 1:
            r(s.int(3, 5), ["impact", "camera_led"], .18, "time", .7, !0);
            break;
        case 2:
            r(s.int(6, 9), ["impact"], .16, "time", .7, !0), r(s.int(14, 17), ["lightdip", "fixture_off"], .45, "time", .65, !0);
            break;
        case 3:
            r(s.int(4, 6), ["lightdip"], .55, "time", .7, !0), r(s.int(9, 12), ["camera_led"], .32, "time", .7, !0);
            break;
        case 4:
            r(s.int(3, 5), ["camera_led"], .32, "time", .7, !0), r(s.int(10, 14), ["lightdip", "fixture_off"], .5, "time", .72, !0);
            break;
        case 5:
            r(s.int(3, 5), ["stutter", "fixture_off"], .7, "time", .72, !0), r(s.int(9, 12), ["blackout_prop"], s.int(12, 17) / 10, "time", .86, !0);
            break;
        case 6:
...
class st {
    ...
    readDocument(e) {
        ...
        const t = ii(`${this.campaign.state.seed}|${this.roomId}`, e, this.campaign.state.index),
            i = si(e);
...
/* OBSERVED 0.14.3 — adaptive mobile input layer */
